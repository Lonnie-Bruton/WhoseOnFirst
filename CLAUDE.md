...
**Frontend:**
- Vanilla JavaScript + HTML/CSS (no framework initially)
...
- **team_members:** id, name, phone (unique, E.164 format), is_active
...
- `/schedule/` - Schedule generation, viewing, manual overrides
  - `GET /schedule/current` - Current week
  - `GET /schedule/upcoming` - Next 4 weeks
  - `POST /schedule/generate` - Trigger regeneration
- `/notifications/` - Notification history and test sends
...
**Out of Scope for Phase 1:** Multi-user auth, email notifications, Teams/Slack integration, mobile app, multi-team support, PTO management.
